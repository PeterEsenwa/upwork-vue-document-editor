	...
	computed: {
		...
		menu() {
			return [
				...
				{
					icon: 'format_clear', title: 'Clear format', disabled: !this.current_text_style, click() {
		...
		async insertPageBreak() {
			...
			for (let i = 0; i < this.content.length; i++) {
				...
				if (match) {
					...
					if (content_plus_tags_close.startsWith('</')) content_plus_tags_close = '<br>' + content_plus_tags_close;
